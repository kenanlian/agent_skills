...
最终计划审查、补丁审查和计划符合性审查由外层控制面分别启动 `review-plan`、`review-patch`、`review-plan-conformance`，不再表示成 `delegate-work` 的 `reviewer` role。运行这些 Review Skill 的顶层只读 Agent 仍可通过 `delegate-work` 委派 bounded explorer 收集证据，但 finding、severity、contract status 和 verdict 始终由该顶层 Review Agent 自己判断。
...
├── execute-plan
└── fresh read-only review-patch + review-plan-conformance
```
...
6. 外层控制面以 fresh read-only 上下文分别运行 `review-patch` 和 `review-plan-conformance`，聚合 verdict、持久化结果，并决定完成或返回原实现会话返工。
